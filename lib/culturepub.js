...
const culturePub = () => {
  ...
  const propertyToSlug = function propertyToSlug(json_metadata, field) {
    ...
    let slug = removeDiacritics(property);
  ...
  const buildImageUrl = function buildThumbnailUrl(json_metadata) {
    ...
    const advertiser_name = propertyToSlug(extra_fields, 'Marque');
    const product_name = propertyToSlug(extra_fields, 'Produit');
  ...
  const isAnAd = function isAnAd(json_metadata) {
    ...
    return advertiser_name !== 'Unknown';
  };
