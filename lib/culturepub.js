...
const culturePub = () => {
  ...
  const getAdMetadata = function getAdMetadata(id) {
    ...
    return rp(uri)
      ...
      .then(function checkScore(ad_metadata) {
        if (scoreIsBelowThreshold(ad_metadata)) {
  ...
  const scoreIsBelowThreshold = function scoreIsAboveThreshold(json_metadata) {
    const votes_count = json_metadata.total_rates;
    const score = Math.round(json_metadata.average_rate);
    const min_score = parseInt(configReader.get('api:min_score'), 10);
    return (votes_count >= 1) && (score <= min_score);
  };
