...
const slackPublisher = function slackPublisher() {
  ...
  const buildPayload = function buildPayload(ad) {
    const message_text = "Today's random ad has been chosen, watch it while " +
      "it's hot!\n_Bot by @pierre — " +
      "<https://github.com/coaxial/adaday|source code>_";
    const icon_emoji = ':tv:';
    const attachment_title = `${ad.advertiser_name}: ${ad.ad_title}`;
    const attachment_link = `${ad.video_url}`;
    const attachment_text = `Originally aired in ${ad.ad_year}, in ` +
      `${ad.ad_country}. Directed by ${ad.ad_director}, created by ` +
      `${ad.ad_agency}.`;
